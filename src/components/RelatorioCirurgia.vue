    ...
    <div class="container">
      ...
      <div class="container-tabela">
        <table class="table is-bordered is-fullwidth">
          ...
          <tbody>
            <tr v-for="cirurgia in filteredCirurgias" :key="cirurgia.RCI_DTHR_INI">
              <td>{{ formatDateTime(cirurgia.RCI_DTHR_INI) }}</td>
              <td>{{ formatDateTime(cirurgia.RCI_DTHR_FIM) }}</td>
              <td>{{ calculateDuration(cirurgia.RCI_DTHR_INI, cirurgia.RCI_DTHR_FIM) }}</td>
      ...
      <div>Média de Tempo entre Cirurgias (Geral): {{ averageTimeBetweenSurgeries }} minutos</div>
...
export default defineComponent({
  ...
  methods: {
    ...
    const differenceInMinutes = (endDate.getTime() - startDate.getTime()) / (1000 * 60); // Diferença em minutos
    return `${Math.floor(differenceInMinutes / 60)}h ${Math.floor(differenceInMinutes % 60)}min`;
  },
    ...
    if (surgeriesCount > 1) {
      this.averageTimeBetweenSurgeries = Math.floor(totalTime / (surgeriesCount - 1));
    } else {
